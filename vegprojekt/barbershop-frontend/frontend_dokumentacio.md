...
│   │   ├── core/
│   │   │   ├── guards/
│   │   │   │   └── auth.guard.ts           ← authGuard, adminGuard, barberGuard, guestGuard
│   │   │   ├── interceptors/
│   │   │   │   └── auth.interceptor.ts     ← JWT token csatolása minden kéréshez
│   │   │   ├── models/
│   │   │   │   ├── barber.model.ts         ← Barber interface
│   │   │   │   ├── booking.model.ts        ← Booking, BookingRequest interface
│   │   │   │   ├── gallery.model.ts        ← GalleryImage interface
│   │   │   │   ├── hairstyle.model.ts      ← Hairstyle interface
│   │   │   │   ├── user.model.ts           ← User, LoginRequest, RegisterRequest stb.
│   │   │   │   └── index.ts                ← barrel export
│   │   │   └── services/
│   │   │       ├── api.service.ts          ← alap HTTP metódusok (get, post, put, delete)
│   │   │       ├── auth.service.ts         ← autentikáció, token kezelés, user state
│   │   │       ├── barber-bookings.service.ts ← fodrász saját foglalásai
│   │   │       ├── barbers.service.ts      ← borbélyok CRUD + schedule + next-slot
│   │   │       ├── bookings.service.ts     ← foglalások CRUD + elérhetőség
...
│   │   │   ├── barber-dashboard/           ← Fodrász dashboard
│   │   │   ├── booking/                    ← Időpontfoglalás
│   │   │   ├── booking-success/            ← Foglalás visszaigazolás
│   │   │   ├── gallery/                    ← Galéria
│   │   │   └── home/                       ← Főoldal
│   │   ├── shared/
│   │   │   └── components/
│   │   │       ├── barber-card/            ← Borbély kártya komponens
│   │   │       ├── footer/                 ← Lábléc
│   │   │       ├── navbar/                 ← Navigációs sáv
│   │   │       └── toast/                  ← Értesítés/toast komponens
│   │   ├── app.component.html
│   │   ├── app.component.scss
│   │   ├── app.component.ts                ← Gyök komponens, layout logika
│   │   ├── app.config.ts                   ← App provider konfiguráció
...
├── tsconfig.app.json                       ← App-specifikus TS konfiguráció
└── tsconfig.spec.json                      ← Teszt TS konfiguráció
```

---

## Technológiai stack (package.json)

### Gyártási függőségek (dependencies)

| Csomag | Verzió | Leírás |
|---|---|---|
| `@angular/animations` | ^17.3.0 | Angular animációs modul |
| `@angular/common` | ^17.3.0 | Közös Angular direktívák, pipe-ok (pl. `*ngIf`, `date`) |
| `@angular/compiler` | ^17.3.0 | Angular template fordító |
| `@angular/core` | ^17.3.0 | Alap Angular framework |
| `@angular/forms` | ^17.3.0 | Reaktív és template-alapú formok (`FormsModule`) |
| `@angular/platform-browser` | ^17.3.0 | Böngésző-platform integráció |
| `@angular/platform-browser-dynamic` | ^17.3.0 | JIT fordítás böngészőben |
| `@angular/router` | ^17.3.0 | Client-side routing |
| `bootstrap` | ^5.3.8 | CSS keretrendszer, reszponzív grid és komponensek |
| `bootstrap-icons` | ^1.13.1 | Ikon készlet (`bi-*` osztályok) |
| `rxjs` | ~7.8.0 | Reaktív programozás (Observable, BehaviorSubject stb.) |
| `tslib` | ^2.3.0 | TypeScript runtime segédkönyvtár |
| `zone.js` | ~0.14.3 | Angular change detection alapja |

### Fejlesztői függőségek (devDependencies)

| Csomag | Verzió | Leírás |
|---|---|---|
| `@angular-devkit/build-angular` | ^17.3.17 | Angular build eszközök |
| `@angular/cli` | ^17.3.17 | Angular parancssori eszköz |
| `@angular/compiler-cli` | ^17.3.0 | AOT fordítás |
| `@types/jasmine` | ~5.1.0 | Jasmine TypeScript típusok |
| `jasmine-core` | ~5.1.0 | Jasmine tesztelő keretrendszer |
| `karma` | ~6.4.0 | Teszt futtatás böngészőben |
| `karma-chrome-launcher` | ~3.2.0 | Chrome indítás Karmával |
| `karma-coverage` | ~2.2.0 | Kód lefedettség mérés |
| `karma-jasmine` | ~5.1.0 | Jasmine–Karma integráció |
| `karma-jasmine-html-reporter` | ~2.1.0 | HTML teszt riport |
| `typescript` | ~5.4.2 | TypeScript fordító |

---

## Környezeti konfiguráció (environments)
...
- `production: false` – fejlesztői módban futtatja az alkalmazást
- `apiUrl` – a lokálisan futó Laravel backend API alap URL-je (port 8000)

### src/environments/environment.prod.ts (produkciós)
...
- `production: true` – optimalizált build és produkciós Angular módot engedélyez
- `apiUrl: '/api'` – relatív útvonal, azaz a frontend és backend ugyanazon a szerveren fut
...
<head>
  <meta charset="utf-8">
  <title>BarbershopFrontend</title>
  <base href="/">
...
- A `<base href="/">` nélkülözhetetlen az Angular Router helyes működéséhez.
- Az `<app-root>` a gyök komponens beillesztési pontja.
- A `viewport` meta tag biztosítja a reszponzív megjelenést mobilon.
...
- Standalone bootstrap módot használ (Angular 17+, nincs AppModule).
- Az `appConfig` tartalmazza az összes provider konfigurációt (router, HTTP client stb.).
- Hiba esetén a konzolra írja a hibaüzenetet.
...
- `provideRouter(routes, ...)` – beregisztrálja a routereket.
- `withInMemoryScrolling` – engedélyezi a horgony-alapú görgetést (`anchorScrolling: 'enabled'`) és visszaállítja a görgetési pozíciót navigációnál (`scrollPositionRestoration: 'enabled'`). Ez teszi lehetővé, hogy a `#barbers` fragment a főoldalon működjön.
- `provideHttpClient(withInterceptors([authInterceptor]))` – globálisan beregisztrálja az auth interceptort, amely minden HTTP kéréshez hozzáadja a JWT tokent.

---

## Útvonalkezelés (app.routes.ts)
...
**Működés:**
- `showLayout` property vezérli, hogy megjelenjen-e a navbar és footer.
- Auth oldalakon (`/login`, `/register` stb.) a navbar és footer el van rejtve, hogy letisztult „kártyás" elrendezés jelenjen meg.
- `ngOnInit`-ban meghívja az `authService.me()` endpointot: ha van érvényes token a localStorage-ban, betölti a bejelentkezett felhasználó adatait az alkalmazás indulásakor.
...
- A `<router-outlet>` az aktuális route komponensét rendereli.
- Auth oldalakon nincs navbar és footer, nincs `min-vh-100` kényszerméretek sem.
- Normál oldalakon a `main` elem `d-flex flex-column min-vh-100` osztályokkal ragasztja alulra a footert.
...
A modellek TypeScript interfészek, amelyek a backend API válaszainak típusát írják le.
...
- `User` – a bejelentkezett vagy listázott felhasználó adatmodellje.
- `role` union típus: csak `'user'`, `'admin'` vagy `'barber'` értéket vehet fel.
- `email_verified_at` nullable: `null` ha nincs megerősítve az email.
- `LoginRequest`, `RegisterRequest`, `ForgotPasswordRequest`, `ResetPasswordRequest` – auth API hívásokhoz szükséges adatobjektumok típusai.
...
- Egyetlen importból elérhetők az összes modell: `import { User, Barber, Booking } from '../models'`.

---

### Guard-ok (src/app/core/guards/auth.guard.ts)
...
export const barberGuard: CanActivateFn = (route, state) => {
  const authService = inject(AuthService);
  const router = inject(Router);
  if (authService.isLoggedIn && (authService.isBarber || authService.isAdmin)) { return true; }
  router.navigate(['/']);
  return false;
};
...
**Guard-ok összefoglalása:**

| Guard neve | Feltétel az átjutáshoz | Elutasítás esetén |
|---|---|---|
| `authGuard` | Be kell legyen jelentkezve | `/login` oldalra irányít, `returnUrl` query paraméterrel |
| `adminGuard` | Bejelentkezett + `role === 'admin'` | Főoldalra irányít |
| `barberGuard` | Bejelentkezett + `role === 'barber'` vagy `'admin'` | Főoldalra irányít |
| `guestGuard` | **Nincs** bejelentkezve | Főoldalra irányít |

- Functional guard stílust használnak (Angular 14.2+), `inject()` segítségével.
- Az `authGuard`-nál a `returnUrl` tárolja, hová szeretett volna jutni a felhasználó, hogy bejelentkezés után oda lehessen visszairányítani.
...
**Működés:**
- Minden kimenő HTTP kérésnél megvizsgálja, van-e `auth_token` a `localStorage`-ban.
- Ha van token, hozzáadja az `Authorization: Bearer <token>` fejlécet.
- `withCredentials: true` – cookie alapú CSRF védelemhez szükséges (Laravel Sanctum).
- Ha nincs token (nem bejelentkezett felhasználó), csak a `withCredentials: true` kerül be a kérésbe.
- Functional interceptor stílust használ (Angular 15+).
...
- Alaposztály, amelyből az összes többi service örököl.
- A `baseUrl` az `environment.apiUrl` értékét veszi fel (fejlesztői vagy produkciós).
- Minden metódus generikus típusparaméterrel (`<T>`) rendelkezik, így típusbiztos Observable-t ad vissza.
- `protected` láthatóság: csak az örökölt osztályokból hívhatók.
...
**Működés:**
1. `ngOnInit()`: kinyeri az `id` és `hash` route paramétereket, és az összes query paramétert (`expires`, `signature`).
2. `HttpClient.get()` direkt hívással GET kérést küld a backend `email/verify/{id}/{hash}` endpointjára a query paraméterekkel együtt.
3. Siker: `successMessage` megjelenítése.
4. Hiba: `errorMessage` megjelenítése (érvénytelen/lejárt link).

**Megjegyzés:** Ez a komponens nem a `AuthService`-t, hanem közvetlenül a `HttpClient`-et használja, mert a verifikációs URL-t a backend generálja, és az `authInterceptor` csatolja a tokent automatikusan.

---

### BarberDashboardComponent (src/app/pages/barber-dashboard/)

**Guard:** `barberGuard` – csak `barber` vagy `admin` role-lal rendelkező bejelentkezett felhasználók érhetik el.
...
**`ngOnInit` logika:**
1. `BarberBookingsService.getMyBarberProfile()` – betölti a bejelentkezett user-hez tartozó borbély profilt.
2. Siker: `loadBookings()` hívása.
3. Hiba: hibaüzenet megjelenítése (ha a usernek nincs borbély profilja).

**`loadBookings()`:**
- `BarberBookingsService.getMyBookings()` – a borbély saját foglalásainak listázása.

**`cancelBooking(id)`:**
- `confirm()` dialógus megjelenítése.
- Jóváhagyás esetén `BarberBookingsService.cancelBooking(id)` → lista újratöltése.

**`getStatusBadgeClass(status)` és `getStatusLabel(status)`:**
- Foglalás státuszhoz Bootstrap badge osztályt és magyar feliratot rendelnek.

| Státusz | Badge osztály | Magyar felirat |
|---|---|---|
| `confirmed` | `bg-success` | Megerősített |
| `pending` | `bg-warning text-dark` | Függőben |
| `completed` | `bg-info` | Teljesített |
| `cancelled` | `bg-danger` | Lemondott |

**Template főbb elemei:**
- Borbély fejléc: fotó/ikon, név, specializáció badge.
- Üres állapot: ikon + „Jelenleg nincs foglalásod."
- Foglalások táblázat: ID, ügyfél neve és elérhetőségei, időpont (`date` pipe), időtartam, státusz badge, „Lemondás" gomb (csak nem lemondott foglalasoknál).
...
**`ngOnInit`:** Párhuzamosan betölti az összes adatot (borbélyok, foglalások, hajstílusok, galéria, felhasználók).

**Tab rendszer:**
- `activeTab` string property tárolja az aktív tabot.
- `setTab(tab)` metódus váltja az aktív tabot.
- A template `*ngIf="activeTab === '...'"` feltétellel mutatja/rejti a tartalmakat.
...
::-webkit-scrollbar-thumb:hover { background: #b8955e; }

/* Global link szín */
a { color: #c8a97e; &:hover { color: #b8955e; } }
```

**Fontos részletek:**
- Bootstrap SCSS forrásból van importálva (`bootstrap/scss/bootstrap`), nem a lefordított CSS-ből, így SCSS változók felülírhatók.
- Bootstrap Icons CSS fájlból importálva.
- Az arany szín (`#c8a97e`) a projekt elsődleges brandingi színe – scrollbar thumb-on és linkeken is megjelenik.
- `scroll-behavior: smooth` – az `anchorScrolling`-el együtt sima görgetést biztosít a hero gomb `#barbers` fragment hivatkozásához.
...
- A projekt Karma + Jasmine tesztelési keretrendszert használ (Angular CLI alapértelmezés).
- `tsconfig.spec.json` konfigurálja a teszt TypeScript beállításokat.
- Teszt fájlok `.spec.ts` kiterjesztéssel kerülnek a komponensek mellé.
